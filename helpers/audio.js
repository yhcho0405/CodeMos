...
export const makeAudioManager = () => {
    ...
    document.addEventListener(
        ...
        ({ isTrusted, metaKey, shiftKey, ctrlKey, altKey, key }) => {
            if (isTrusted && !(metaKey || shiftKey || ctrlKey || altKey) && key !== "Escape" && key !== "Esc") {
                _initialize();
            }
        },
        { once: true }
    ...
    async function _playTrack(audioBuffer, loop = true) {
        ...
        if (hasInitialized) {
            return Promise.all([audioCTX.resume(), audioBuffer]).then((e) => playBuffer(e[1]));
        } else {
            return Promise.all([_initialize(), audioBuffer]).then((e) => playBuffer(e[1]));
        }
    }
